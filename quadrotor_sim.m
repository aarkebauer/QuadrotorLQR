...
global T Tmax m g M Ax Ay Az C J l k b Ixx Iyy Izz w1 w2 w3 w4 ...
    ...
    B R_cost Q_cost P int_out K_int ...
...
sim_time = 30; % simulation runtime in seconds
...
%% calculate gain (K) for LQI controller and associated algebraic riccati equation (S)
% e gives corresponding eigenvalues
% C_sys = eye(12);% (C is called something else in quadrotor_ode.m)
% D = zeros(12,4);
% [K_int,S,e] = lqi(ss(A,B,C_sys,D),Q_cost,R_cost,N_cost); - https://www.mathworks.com/help/control/ref/lqi.html
% [K_int,S,e] = lqi(ss(A,B,C_sys,D),Q_cost,R_cost);
% [K_int,S,e] = lqi(ss(A,B,C_sys,D),[eye(12) zeros(12);zeros(12) zeros(12)],eye(4));
